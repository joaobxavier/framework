 ...
 */
public class EpsProducersMutationImpact extends ModelHandler {
	...
	static String outputDirectory = "c:/results/EpsProducersMutationImpact_Nomut2---/";
	...
	protected static float finitshITerationTime = 720f;
	...
	protected static float mutationTime = 240f;

	protected static float neighborhoodRaius = systemSize * 0.06f;

	protected static boolean mutationFlag = true;
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		ParticulateSpecies activeWTNeighbors = new ParticulateSpecies(
				"activeWTNeighbors", specificMassX, Color.green);
		...
		// EPS -
		ParticulateSpecies[] c2 = { activeWT, eps_WT, activeWTNeighbors,
				activeEpsMinusMutant, activeControlMutant };
		float fXVol_WT = 1.0f / (1.0f + fEPS_WT * specificMassX
				/ specificMassEPS_WT);
		float[] f2 = { 1.0f, 0, 0, 0, 0 };
		BiomassSpecies wildType;
		if (mutationFlag) {
			wildType = new MutatorBiomassSpeciesForImpact("wildType", c2, f2,
					activeWT, activeEpsMinusMutant, activeWTNeighbors,
					mutationTime, neighborhoodRaius);
		} else {
			wildType = new MutatorBiomassSpeciesForImpact("wildType", c2, f2,
					activeWT, activeControlMutant, activeWTNeighbors,
					mutationTime, neighborhoodRaius);
		}
		...
		// WT Neighbors of mutant
		Reaction sUptakeWTNeighbors = new Reaction("sUptakeWTNeighbors",
				activeWTNeighbors, qSMax, 1);
		sUptakeWTNeighbors.addFactor(mO);
		// WT control mutant
		...
		// active mass EPS + of neighbors of the mutant
		NetReaction rsXactiveWTNeighbors = new NetReaction(1);
		rsXactiveWTNeighbors.addReaction(sUptakeWTNeighbors, Y * 1
				/ (1 + fEPS_WT));
		activeWTNeighbors.setProcesses(rsXactiveWTNeighbors);
		...
		// EPS
		NetReaction rsEps_WT = new NetReaction(3);
		rsEps_WT.addReaction(sUptakeWT, Y * fEPS_WT / (1 + fEPS_WT));
		rsEps_WT.addReaction(sUptakeWTNeighbors, Y * fEPS_WT / (1 + fEPS_WT));
		...
		rsOxygen.addReaction(sUptakeWTNeighbors, -YO * (1 - Y));
	...
	public static void run() {
		...
		ApplicationComponent app = new EpsProducersMutationImpact();
