 ...
 */
public class Monod_Sum_of_TwoSubstrates extends ProcessFactor {
	 ...
	 * @param k
	 */
	public Monod_Sum_of_TwoSubstrates(
		SoluteSpecies c1,
		//float ks1,
		SoluteSpecies c2,
		float ks1) {
	...
	public float getValue() {
		...
		return (conc1 + conc2) / (_ks1 + conc1 + conc2);
	}
	...
	public float getDerivative(SoluteSpecies c) {
		if (c == _solute1) {
			float conc1 = _solute1.getValue();
			float conc2 = _solute2.getValue();
			return 1 / (conc1 + conc2 + _ks1) - (conc1 + conc2) / ((conc1 + conc2 + _ks1)*(conc1 + conc2 + _ks1));
			//return 
				// _ks1
				// / (_ks1 + conc1)
				// / (_ks1 + conc1)
