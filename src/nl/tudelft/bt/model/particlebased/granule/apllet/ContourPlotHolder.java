...
public class ContourPlotHolder {
	...
	private static int numX = Monospecies2DGranuleModel.GRIDSIZE;

	/** The number of y values in the dataset. */
	private static int numY = Monospecies2DGranuleModel.GRIDSIZE;
	...
	private JFreeChart createContourPlot() {
		...
		//ColorPalette pal = new RainbowPalette();
		//pal.invertPalette();
	...
	private void createDataset() {
		...
		float stepX = Monospecies2DGranuleModel.SYSTEMSIZE / numX;
		float stepY = Monospecies2DGranuleModel.SYSTEMSIZE / numY;

		int c = 0;
		for (int i = 0; i < numX; i++) {
			for (int j = 0; j < numY; j++) {
				//				xValues[c] = new Double(stepX/2 + i * stepX);
				//				yValues[c] = new Double(stepY/2 + i * stepY);
