/**
 * @(#)QuickTimeWriter.java  1.1  2011-01-04
 *
 ...
 * consists of a frame containing one PCM impulse per channel. Samples of
 * compressed media data may encompass larger time units.
 * <p>
 ...
 * track stores the media starting and ending at the nearest sync frames.
 * <p>
 ...
 * must be interleaved from time to time. An interleave should occur about twice
 ...
 * by one second.
 * <p>
 * For convenience, this class provides encoders for video frames in the following
 * formats: RAW, JPEG and PNG. Media data in other formats, including all audio
 ...
 * @version 1.1 2011-01-04 Adds "depth" parameter to addVideoTrack method.
...
public class QuickTimeWriter {
    ...
    private abstract class Track {
        ...
        /* chunk offset atom -------- */
            ...
            if (chunks.size() == 0 || chunks.get(chunks.size() - 1).getChunkOffset() <= 0xffffffffL) {
                ...
                // offset table.
                for (Chunk c : chunks) {
                    d.writeUInt(c.getChunkOffset()); // offset
    ...
    private class WideDataAtom extends Atom {
        ...
        public void setSize(long explicitSize) {
     ...
     * than 1.
     */
    public void addVideoTrack(VideoFormat encoding, long timeScale, int width, int height) {
     ...
     */
    public void addVideoTrack(String compressionType, String compressorName, long timeScale, int width, int height, int depth, int syncInterval) {
     ...
     */
    public void addAudioTrack(AudioFormat format) {
    ...
    public void addAudioTrack(String compressionType, //
            ...
            int frameDuration, int frameSize) {
        if (compressionType == null || compressionType.length() != 4) {
    ...
    public void writeFrame(int track, BufferedImage image, long duration) throws IOException {
        ...
        switch (t.videoEncoding) {
            case RAW: {
                ...
                switch (t.videoDepth) {
                    case 8: {
                        ...
                        // FIXME - We blindly assume that the bytes exactly much the width and height of the image
                        mdatAtom.getOutputStream().write(bytes);
                        break;
                    }
                    case 24: {
                        WritableRaster raster = image.getRaster();
                        int[] raw = new int[t.videoWidth * 3]; // holds a scanline of raw image data with 3 channels of 32 bit data
                        ...
                        for (int y = 0; y < t.videoHeight; y++) {
                            raster.getPixels(0, y, t.videoWidth, 1, raw);
            ...
            case RLE: {
                isSync = t.previousData == null || t.sampleCount % t.syncInterval == 0;
                        WritableRaster raster = image.getRaster();
                if (t.encoder == null) {
                ...
                switch (t.videoDepth) {
                    case 16: {
                        ...
                        // FIXME - We blindly assume that the data exactly much the width and height of the image
                        if (isSync) {
                            enc.writeKey16(out, data, 0, image.getWidth(), image.getWidth());
                        } else {
                            enc.writeDelta16(out, data, (short[]) t.previousData, 0, image.getWidth(), image.getWidth());
                        }
                    ...
                    case 24: {
                        ...
                        // FIXME - We blindly assume that the data exactly much the width and height of the image
                        if (isSync) {
                            enc.writeKey24(out, data, 0, image.getWidth(), image.getWidth());
                        } else {
                            enc.writeDelta24(out, data, (int[]) t.previousData, 0, image.getWidth(), image.getWidth());
                        }
                        if (t.previousData == null) {
                    ...
                    case 32: {
                        ...
                        // FIXME - We blindly assume that the data exactly much the width and height of the image
                        if (isSync) {
                            enc.writeKey32(out, data, 0, image.getWidth(), image.getWidth());
                        } else {
                            enc.writeDelta32(out, data, (int[]) t.previousData, 0, image.getWidth(), image.getWidth());
                        }
                        if (t.previousData == null) {
                            t.previousData = new int[data.length];
                        }
        ...
        // XXX - This is a dirty hack, because we directly pass the underlying
        // output stream to the RLE encoder.
        mdatAtom.setSize(getRelativeStreamPosition() - mdatAtom.getOffset() - 8);
